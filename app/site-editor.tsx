  ...
  return <div className="editor-body">
    <Tabs value={tab} onValueChange={setTab}>
      ...
      <div className="editor-scroll"><fieldset disabled={saving} style={{border:0,padding:0,margin:0,minWidth:0}}>
        ...
        <TabsContent value="events"><div className="editor-group"><p className="editor-help">Οι εργασίες εμφανίζονται αυτόματα ομαδοποιημένες ανά μήνα, από την πιο πρόσφατη στην παλαιότερη. Η εικόνα και ο σύνδεσμος είναι προαιρετικοί.</p>
          {draft.events.map((event,i)=><div className="editor-record" key={event.id}>{tools("events",i,`Εργασία ${i+1}`)}<Field label="Ημερομηνία" value={event.date} type="date" onChange={v=>set("events",draft.events.map((o,n)=>n===i?{...o,date:v}:o))}/><Field label="Εργασία / τελετή / ομιλία" value={event.title} onChange={v=>set("events",draft.events.map((o,n)=>n===i?{...o,title:v}:o))}/><Field label="Σύνδεσμος εικόνας (προαιρετικό)" value={event.image} type="url" placeholder="https://…" onChange={v=>set("events",draft.events.map((o,n)=>n===i?{...o,image:v}:o))}/><Field label="Σύνδεσμος / λινκ (προαιρετικό)" value={event.link} type="url" placeholder="https://…" onChange={v=>set("events",draft.events.map((o,n)=>n===i?{...o,link:v}:o))}/></div>)}
          <button className="add-button" disabled={draft.events.length>=150} onClick={()=>set("events",[...draft.events,{id:crypto.randomUUID(),date:"",title:"",image:"",link:""}])}><Plus size={18}/>Προσθήκη εργασίας</button>
        </div></TabsContent>
